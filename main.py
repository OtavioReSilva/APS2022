...
def CadastroDespejo(empresa, cnpj, tipo, quantidade, regiao):
  global client
  client.send(str("#!cadastroDespejo!# " + str(empresa) + "  :  " + str(cnpj) + "  :  " + str(tipo) + "  :  " + str(quantidade) + "  :  " + str(regiao)).encode('UTF-8'))
...
@eel.expose
def BuscaTipoDespejo():
  global client
  client.send(str("#!BuscaTipoDespejo!# " )).encode('UTF-8')
